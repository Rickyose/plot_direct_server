...
if [ $initiate_start -eq 1 ]; then
	...
	mkdir /gdrive2/cha/
	sudo -u root rclone mount --allow-non-empty --daemon gdrive3: /gdrive2 && chmod 777 /gdrive3 && chown -R root /gdrive3
	mkdir /gdrive3/cha/
	while [ 1 -gt 0 ]
	do
		if [ $plot_number -eq 3 ]; then
			sudo fstrim -v /plot1
			plot_number=0
		fi
		sleep 30s
		...
		####################
		 ...
		 new_total_plot=$((($gdrive1_total_plot + $gdrive2_total_plot + $gdrive3_total_plot)*1))
